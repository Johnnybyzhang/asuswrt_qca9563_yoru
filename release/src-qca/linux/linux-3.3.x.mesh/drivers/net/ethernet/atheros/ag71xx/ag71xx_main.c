					  ...
					  struct net_device *dev)
{
	...
	if (unlikely(used >= (size >> 1))) {
		struct ag71xx_platform_data *pdata = ag71xx_get_pdata(ag);
		int tx_done;

		tx_done = ag71xx_tx_packets(ag, dev, pdata->is_ar7240);
		ag71xx_debugfs_update_napi_stats(ag, 0, tx_done);
		used = ring->used;
	}
...
static u32
__mii_read(struct mii_bus *bus, int reg)
{
	u16 r1, r2, page;
	u16 lo, hi;
...
static void
__mii_write(struct mii_bus *bus, int reg, u32 val)
{
	u16 r1, r2, r3;
	u16 lo, hi;
...
static int athr_phy_ctrl(struct ag71xx *ag, struct net_device *dev,
			 struct ifreq *ifr, int cmd)
{
	...
	if (unlikely(!bus) || cmd != ATHR_PHY_CTRL_IOC)
		return -EINVAL;
	...
	case ATHR_PHY_RD:
		if (ethcfg->portnum != 0x3f) {
			ethcfg->val = mdiobus_read(bus, ethcfg->portnum, ethcfg->phy_reg);
		} else {
			ethcfg->val = __mii_read(bus, ethcfg->phy_reg);
		}
		break;

	case ATHR_PHY_WR:
		if (ethcfg->portnum != 0x3f) {
			mdiobus_write(bus, ethcfg->portnum, ethcfg->phy_reg, ethcfg->val);
		} else {
			__mii_write(bus, ethcfg->phy_reg, ethcfg->val);
		}
		break;

	default:
		printk("%s: unknown ethcfg->cmd %x\n", __func__, cmd);
		break;
	}
...
static int ag71xx_do_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case SIOCSMIIREG:
        ...
        case ATHR_PHY_CTRL_IOC:
            ret = athr_phy_ctrl(ag, dev, ifr, cmd);
            return ret;
...
static int __devinit ag71xx_probe(struct platform_device *pdev)
{
	...
	pr_info("%s: Atheros AG71xx at 0x%08lx, irq %d, MAC %02X:%02X:%02X:%02X:%02X:%02X\n",
