...
#ifdef AC55U_FIX
u8 init_time_WAN_mask = 0x42; // port 1 & port 6
#endif
...
struct ar8xxx_chip {
	...
	void (*setup_port)(struct ar8216_priv *priv, int port, u32 egress,
			   u32 ingress, u32 members, u32 pvid, u32 pprio);
	...
	void (*vtu_load_vlan)(struct ar8216_priv *priv, u32 vid, u32 vprio, u32 port_mask);
...
#ifdef AC55U_FIX // these two fields for 8337
...
ar8216_mib_fetch_port_stat(struct ar8216_priv *priv, int port, bool flush)
{
	...
	for (i = 0; i < priv->chip->num_mibs; i++) {
		...
		if ((!strcmp(mib->name, "RxGoodByte") || !strcmp(mib->name, "TxByte")) && (port >= 1 ||port <= 5)) {
			mib_stats[i] = 0;
			continue;
		}
...
static void
ar8216_vtu_load_vlan(struct ar8216_priv *priv, u32 vid, u32 vprio, u32 port_mask)
{
	u32 op;
...
ar8216_setup_port(struct ar8216_priv *priv, int port, u32 egress, u32 ingress,
		  u32 members, u32 pvid, u32 pprio)
{
	u32 header;
...
#ifdef AC55U_FIX
		mdiobus_write(bus, i, MII_CTRL1000, ADVERTISE_1000FULL | ADVERTISE_1000HALF);
#else
...
#ifdef AC55U_FIX
		t |= cfg->sgmii_txclk_phase_sel << AR8337_PAD_SGMII_TXCLK_SEL_S;
		t |= cfg->sgmii_rxclk_phase_sel << AR8337_PAD_SGMII_RXCLK_SEL_S;
#endif
		/*
...
static int ar8327_hw_init(struct ar8216_priv *priv)
{
...
#if defined(CONFIG_PLAC56) || defined(CONFIG_PLAC66U) //for PL-AC56 & PL-AC66U
	t|=0x80000000;
#endif
	...
	for (i = 0; i < AR8327_NUM_PHYS; i++) {
		mdiobus_write(bus, i, AR8216_REG_CTRL, 0x8000);		// the 0x1800 or 0x9800 seem reset incomplete (bit11 for power down)
	}
	msleep(1);
	for (i = 0; i < AR8327_NUM_PHYS; i++) {
		...
		mdiobus_write(bus, i, MII_ADVERTISE, ADVERTISE_ALL |
						     ...
						     ADVERTISE_PAUSE_ASYM);
#ifdef AC55U_FIX
		mdiobus_write(bus, i, MII_CTRL1000, ADVERTISE_1000FULL | ADVERTISE_1000HALF);	// rt-ac55u use this
#endif
	}

#ifdef AC55U_FIX
	t = priv->read(priv, AR8337_REG_SGMII_CTRL);
	t |= (AR8337_SGMII_EN_TX | AR8337_SGMII_EN_RX | AR8337_SGMII_EN_PLL);
	priv->write(priv, AR8337_REG_SGMII_CTRL, t);
#endif
	msleep(1000);
...
#ifndef AC55U_FIX // PAD5 won't be used in AC55U
...
#ifdef AC55U_FIX // port 1 is WAN in AC55U
		ar8216_sw_reg_set(priv, 0x978, 0x1e864443);
#else
...
#ifdef AC55U_FIX // port 5 is normal port in AC55U
		ar8216_sw_reg_set(priv, 0x998, 0x19008643);
#else
...
static void
ar8327_vtu_load_vlan(struct ar8216_priv *priv, u32 vid, u32 vprio, u32 port_mask)
{
	u32 op;
	u32 val;
	int i;
...
#ifdef AC55U_FIX
#define AR8337_VTU_PRI_OVER_EN (1 << 3)
	if(vprio & 0x7)
		val |= (vprio & 0x7) | AR8337_VTU_FUNC0_PRI_OVER_EN;
#endif	/* AC55U_FIX */
		  ...
		  u32 members, u32 pvid, u32 pprio)
{
	...
	t |= pprio << AR8327_PORT_VLAN0_DEF_SPRIO_S;
	t |= pvid << AR8327_PORT_VLAN0_DEF_CVID_S;
	t |= pprio << AR8327_PORT_VLAN0_DEF_CPRIO_S;
...
#ifdef AC55U_FIX
static int
ar8216_sw_set_vpri(struct switch_dev *dev, const struct switch_attr *attr,
		  struct switch_val *val)
{
	struct ar8216_priv *priv = to_ar8216(dev);
	priv->vlan_prio[val->port_vlan] = val->value.i;
	return 0;
}

static int
ar8216_sw_get_vpri(struct switch_dev *dev, const struct switch_attr *attr,
		  struct switch_val *val)
{
	struct ar8216_priv *priv = to_ar8216(dev);
	val->value.i = priv->vlan_prio[val->port_vlan];
	return 0;
}
#endif	/* AC55U_FIX */
	...
	for (i = 0; i < val->len; i++) {
		...
		if (p->flags & (1 << SWITCH_PORT_FLAG_TAGGED)) {
			...
			for (j = 0; j < AR8X16_MAX_VLANS; j++) {
				if (j == val->port_vlan)
					continue;
				if(priv->vlan_table[j] & (1 << p->id))
					printk("WARNING !! vlan_table[%d] (%08x) bit %d is removed !!!!\n", j, priv->vlan_table[j], p->id);
...
ar8216_sw_hw_apply(struct switch_dev *dev)
{
	...
	if (!priv->init) {
		...
		for (i = 0; i < dev->ports; i++) {
#ifdef AC55U_FIX // executed only at initial time (priv->init==true)
			u8 mask = (1 << i);
			if (mask & init_time_WAN_mask) // WAN
				portmask[i] = init_time_WAN_mask & (~mask);
			else
				portmask[i] = ((~init_time_WAN_mask)&0x3f) & (~mask);
#else
	...
	for (i = 0; i < dev->ports; i++) {
		...
		if (priv->vlan) {
			...
			pprio = priv->vlan_prio[priv->pvid[i]];

#if defined(CONFIG_MAPAC1750)
			...
			if ((1<<i) & init_time_WAN_mask) // WAN
				...
				pvid = priv->vlan_id[priv->pvid[i]];
			egress = AR8216_OUT_STRIP_VLAN; // remove VLAN tag at default
#else
			pvid = 0;
			egress = AR8216_OUT_KEEP;
#endif
...
ar8216_sw_reset_switch(struct switch_dev *dev)
{
	...
	memset(&priv->vlan_prio, 0, sizeof(priv->vlan_prio));
...
#ifdef AC55U_FIX
#define CONTROL_WHICH_PAD  AR8327_REG_PAD0_MODE  // SGMII should be controlled in PAD0, but PAD5's RX delay could affect MAC0,MAC5 & MAD6?
//#define CONTROL_WHICH_PAD  AR8327_REG_PAD5_MODE 
static int
pad0_SGMII_set_txdelay(struct switch_dev *dev, const struct switch_attr *attr,
		   struct switch_val *val)
{
	struct ar8216_priv *priv = to_ar8216(dev);
	int tmpval= val->value.i;
	u32 t;
	if ((tmpval < 0) || (tmpval > 4))
		return -EINVAL;
	t=priv->read(priv, CONTROL_WHICH_PAD);
	t &= ~(AR8327_PAD_RGMII_TXCLK_DELAY_EN | AR8327_PAD_RGMII_TXCLK_DELAY_SEL);
	if (tmpval)
		t |= AR8327_PAD_RGMII_TXCLK_DELAY_EN | ((tmpval-1) << AR8327_PAD_RGMII_TXCLK_DELAY_SEL_S);
	priv->write(priv, CONTROL_WHICH_PAD, t);
	return 0;
}

static int
pad0_SGMII_get_txdelay(struct switch_dev *dev, const struct switch_attr *attr,
		   struct switch_val *val)
{
	struct ar8216_priv *priv = to_ar8216(dev);
	u32 t;
	t=priv->read(priv, CONTROL_WHICH_PAD);
	if (t & AR8327_PAD_RGMII_TXCLK_DELAY_EN)
		val->value.i=((t&AR8327_PAD_RGMII_TXCLK_DELAY_SEL)>>AR8327_PAD_RGMII_TXCLK_DELAY_SEL_S) + 1;
	else
		val->value.i=0;
	return 0;
}
...
static int
pad0_SGMII_set_rxdelay(struct switch_dev *dev, const struct switch_attr *attr,
		   struct switch_val *val)
{
	struct ar8216_priv *priv = to_ar8216(dev);
	int tmpval= val->value.i;
	...
	if ((tmpval < 0) || (tmpval > 4))
		return -EINVAL;
	t=priv->read(priv, CONTROL_WHICH_PAD);
	t &= ~(AR8327_PAD_RGMII_RXCLK_DELAY_EN | AR8327_PAD_RGMII_RXCLK_DELAY_SEL);
	if (tmpval)
		t |= AR8327_PAD_RGMII_RXCLK_DELAY_EN | ((tmpval-1) << AR8327_PAD_RGMII_RXCLK_DELAY_SEL_S);
	priv->write(priv, CONTROL_WHICH_PAD, t);
	return 0;
}

static int
pad0_SGMII_get_rxdelay(struct switch_dev *dev, const struct switch_attr *attr,
		   struct switch_val *val)
{
	struct ar8216_priv *priv = to_ar8216(dev);
	u32 t;
	t=priv->read(priv, CONTROL_WHICH_PAD);
	if (t & AR8327_PAD_RGMII_RXCLK_DELAY_EN)
		val->value.i=((t&AR8327_PAD_RGMII_RXCLK_DELAY_SEL)>>AR8327_PAD_RGMII_RXCLK_DELAY_SEL_S) + 1;
	else
		val->value.i=0;
	return 0;
}
...
static int
ar8216_set_port_power(struct switch_dev *dev, const struct switch_attr *attr,
		   struct switch_val *val)
{
	struct ar8216_priv *priv = to_ar8216(dev);
	struct mii_bus *bus;
	...
	bus = priv->phy->bus;
	t = mdiobus_read(bus, port-1, AR8216_REG_CTRL);
	if (val->value.i) // power on
		t = 0x1000;	// autonegotiation
	else
		t = 0x0800;	// fix 0x8800 not really power down a port
	mdiobus_write(bus, port-1, AR8216_REG_CTRL, t);
	return 0;
}

static int
ar8216_get_port_power(struct switch_dev *dev, const struct switch_attr *attr,
		   struct switch_val *val)
{
	struct ar8216_priv *priv = to_ar8216(dev);
	struct mii_bus *bus;
...
static struct switch_attr ar8216_globals[] = {
...
#ifdef AC55U_FIX
	{
		.type = SWITCH_TYPE_INT,
		.name = "SGMII_tx_delay",
		.description = "0:disable 1-4:enable by setting DELAY_SEL 0-3 on PAD0",
		.set = pad0_SGMII_set_txdelay,
		.get = pad0_SGMII_get_txdelay,
	},
	{
		.type = SWITCH_TYPE_INT,
		.name = "SGMII_rx_delay",
		.description = "0:disable 1-4:enable by setting DELAY_SEL 0-3 on PAD0",
		.set = pad0_SGMII_set_rxdelay,
		.get = pad0_SGMII_get_rxdelay,
	},
#endif
};
...
static struct switch_attr ar8216_port[] = {
...
#ifdef AC55U_FIX
	{
		.type = SWITCH_TYPE_INT,
		.name = "power",
		.description = "port's power control",
		.set = ar8216_set_port_power,
		.get = ar8216_get_port_power,
	},
#endif
};
...
static struct switch_attr ar8216_vlan[] = {
...
#ifdef AC55U_FIX
	{
		...
		.set = ar8216_sw_set_vpri,
		.get = ar8216_sw_get_vpri,
		.max = 7,
	},
#endif /* AC55U_FIX */
};
...
ar8216_config_init(struct phy_device *pdev)
{
	...
	if (chip_is_ar8316(priv)) {
		...
		swdev->name = "Atheros AR8327/AR8337";
		swdev->vlans = AR8X16_MAX_VLANS;
