...
void plat_dev_init(void)
{
	...
	int	r;
	uchar	sectorBuff[0x4000];	/* One RADIO Calibration Data occupies 0x4000 bytes. */

	if ((r = ra_factory_read(sectorBuff, WLANCAL, sizeof(sectorBuff)))) {
		printf("%s: read %d bytes from offset %x of WLANCAL fail! (r = %d)\n",
			__func__, sizeof(sectorBuff), WLANCAL, r);
	}
	calData = (volatile u16*) &sectorBuff[0];
#else
	u32	CalAddr = WLANCAL;
	...
	*/
	...
	if ((is_ar7241() || is_ar7242()))
		ath_pci_write_config(&hose, pci_dev, 0x10, 0x1000ffff);
	else
		ath_pci_write_config(&hose, pci_dev, 0x10, 0xffff);

	ath_pci_write_config(&hose, pci_dev, 0x04, 0x6);
